...
Tais células atuam como filtros locais sobre o espaço de entrada e quanto mais complexa a célula, maior o seu campo receptivo. As redes convolutivas segue esse mesmo princípio de campos receptivos, o que ajuda a diminuir o custo computacional de treinamento.
...
\end{itemize}
Os kernels são parâmetros ajustáveis do modelo, eles devem aprender a modelar relações entre pixels vizinhos, independente da posição em que esses pixels se encontram na imagem. Essa extração funciona usando uma convolução, similar aos primeiros extratores de características feitos manualmente em visão computacional.  A convolução o ocorre durante a fase \textit{foward}, onde cada um desses kernels são convoluídos com a entrada. 
