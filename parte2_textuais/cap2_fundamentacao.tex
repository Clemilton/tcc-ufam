...
\begin{itemize}
	...
	\item \textit{Agrupamento}: o objetivo é distribuir as séries temporais em grupos, de modo que séries temporais de um mesmo grupo são mais similares entre si, do que  a séries temporais de  outros grupos, de acordo com uma função de dissimilaridade; 
...
\subsection{Medidas de dissimilaridade}

Em mineração de séries temporais, para tarefas como classificação, agrupamento e indexação, é importante definir a função de distância usada para calcular a dissimilaridade entre pares de séries temporais. A seguir, são apresentadas as principais duas funções de distância para séries temporais propostas na literatura.
...
\begin{itemize}
	\item \textit{Baseados em atributos: } utilizam valores de "baixo-nível" para extrair atributos temporais de "maior nível", os quais podem ser considerados sob uma ótica de mineração de dados "clássica".
\end{itemize}
...
Diversos estudos na área evidenciam que a medida de distância Dynamic Time Warping (DTW) é uma das medidas mais adequadas para comparação séries temporais, possibilitando a identificação de padrões e a recuperação por conteúdo. \cite{ratanamahatana2004} \cite{alon2009} \cite{Berndt:1994:UDT:3000850.3000887}
