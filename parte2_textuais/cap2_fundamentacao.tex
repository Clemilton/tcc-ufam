...
Um exemplo de classificador baseado em instâncias é o $k$-NN. Diversos estudos evidenciam que a distância DTW combinada com um classificador 1-NN é uma das abordagens mais competitivas em tarefas de classificação \cite{Wang2013} \cite{Lines2015} \cite{giusti2013}. Contudo existem algumas desvantagens. O 1-NN é um classificador do tipo "preguiçoso" pois ele precisa verificar toda a base de treino para classificar uma instância. O cálculo da DTW tem complexidade $O(nm)$, onde $n$ e $m$ são os tamanhos das séries temporais. Ou seja, para cada passo do 1-NN é feito um cálculo da DTW, o que provoca um tempo muito grande para classificar toda as instâncias de teste, especialmente se o tamanho das séries forem grandes.
